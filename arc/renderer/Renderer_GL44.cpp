...
namespace arc { namespace renderer {
	...
	GeometryConfigID Renderer_GL44::geometry_config_register(const VertexLayout& layout, IndexType index_type, PrimitiveType primitive_type)
	{
		VertexLayout* vl_ptr = nullptr;
		uint32_t idx;
		...
		if (!vl_ptr)
		{
			vl_ptr = copy(layout, *m_alloc);
			m_vertex_layouts.push_back(vl_ptr);

			GeometryConfig* gc;
			tie(idx,gc) = m_geometry_config_data.create();
			gc->gl_vao = m_vao_gl_ids[idx];
			gc->index_type = index_type;
			gc->layout = vl_ptr;
			gc->primitive = primitive_type;
		}
		else
		{
			idx = m_geometry_config_data.find( [&](const GeometryConfig& gc) {
				return gc.layout == vl_ptr && gc.index_type == index_type && gc.primitive == primitive_type;
			});

			if (!m_geometry_config_data.valid(idx))
			{
				GeometryConfig* gc;
				tie(idx, gc) = m_geometry_config_data.create();
				gc->gl_vao = m_vao_gl_ids[idx];
	...
	{
		ARC_NOT_IMPLEMENTED; // TODO we need reference counting here
	}
